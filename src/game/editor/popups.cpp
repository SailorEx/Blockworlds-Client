...
void CEditor::UiDoPopupMenu()
{
	for (int i = 0; i < g_UiNumPopups; i++)
	{
		...
		/*
		// close popup
		if (pEditor->Input()->KeyDown(KEY_ESCAPE) /*||
				(!Inside && UI()->ActiveItem() != &s_UiPopups[i].m_pId &&
				(UI()->MouseButtonClicked(0) || UI()->MouseButtonClicked(2) || UI()->MouseButtonClicked(1))))
		{
			g_UiNumPopups--;
			UI()->SetActiveItem(0);
		}
...
int CEditor::PopupEvent(CEditor* pEditor, CUIRect View)
{
	...
	if (pEditor->m_PopupEventType == POPEVENT_EXIT)
		...
		pEditor->UI()->DoLabel(&Label, "Quit the client", 20.0f, 0);
	...
	if (pEditor->m_PopupEventType == POPEVENT_EXIT)
		...
		pEditor->UI()->DoLabel(&Label, "The map contains unsaved data, you might want to save it before you quit the client.\nContinue anyway?", 10.0f, -1, Label.w - 10.0f);
	else if (pEditor->m_PopupEventType == POPEVENT_LOAD)
